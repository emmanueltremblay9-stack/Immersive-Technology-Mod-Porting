...
public class ITRecipes extends RecipeProvider {
    ...
    public ITRecipes(PackOutput pOutput, CompletableFuture<HolderLookup.Provider> registries) { super(pOutput, registries); }

    @Override protected void buildRecipes(@NotNull RecipeOutput consumer) {
        multiblockRecipes();
    ...
    private void itemRecipes(@Nonnull RecipeOutput consumer) {
        ...
        ShapedRecipeBuilder.shaped(RecipeCategory.MISC, ITBlocks.Metal.VALVE_LIMITER.get().asItem(), 2).define('L', IETags.getTagsFor(EnumMetals.IRON).plate).define('R', IEBlocks.Connectors.CONNECTOR_REDSTONE.get()).define('P', Objects.requireNonNull(BuiltInRegistries.BLOCK.get(ResourceLocation.fromNamespaceAndPath("immersiveengineering", "conveyor_basic")))).define('I', IEItems.Ingredients.COMPONENT_IRON.get()).define('C', IEItems.Ingredients.CIRCUIT_BOARD.get()).pattern("LRL").pattern("PIP").pattern("LCL").unlockedBy("has_iron_plate", has(IETags.getTagsFor(EnumMetals.IRON).plate)).save(consumer, toResourceLocation("valve_limiter"));
    ...
    private void recipesBoilerTank(@Nonnull RecipeOutput out) {
    ...
    private void recipesBoilerLiquid(@Nonnull RecipeOutput out) {
    ...
    private void recipesBoilerSolid(@Nonnull RecipeOutput out) {
    ...
    private void recipesCoolingTower(@Nonnull RecipeOutput out) {
    ...
    private void recipesDistiller(@Nonnull RecipeOutput out) {
        ItemStack salt = new ItemStack(ITItems.SALT.get(), 1);
        DistillerRecipeBuilder.builder(SizedFluidIngredient.of(FluidTags.WATER, 1000), new FluidStack(ITFluids.DISTILLED_WATER.getStill(), 500), 20, 10000).addItemOutput(salt, 0.5f).build(out, toResourceLocation("distiller/water"));
    }

    private void recipesHeatExchanger(@Nonnull RecipeOutput out) {
        HeatExchangerRecipeBuilder.builder(SizedFluidIngredient.of(FluidTags.WATER, 250), SizedFluidIngredient.of(ITTags.fluidFlueGas, 1000), new FluidStack(ITFluids.STEAM.getStill(), 450), null, 640, 10).build(out, toResourceLocation("heat_exchanger/water_fluegas"));
        HeatExchangerRecipeBuilder.builder(SizedFluidIngredient.of(ITTags.fluidDistilledWater, 250), SizedFluidIngredient.of(ITTags.fluidFlueGas, 1000), new FluidStack(ITFluids.STEAM.getStill(), 500), null, 640, 10).build(out, toResourceLocation("heat_exchanger/distwater_fluegas"));
        HeatExchangerRecipeBuilder.builder(SizedFluidIngredient.of(FluidTags.WATER, 250), SizedFluidIngredient.of(ITTags.fluidMoltenSalt, 80), new FluidStack(ITFluids.STEAM.getStill(), 450), new FluidStack(ITFluids.HEATED_SALT.getStill(), 80), 640, 10).build(out, toResourceLocation("heat_exchanger/water_moltensalt"));
        HeatExchangerRecipeBuilder.builder(SizedFluidIngredient.of(ITTags.fluidDistilledWater, 250), SizedFluidIngredient.of(ITTags.fluidMoltenSalt, 80), new FluidStack(ITFluids.STEAM.getStill(), 500), new FluidStack(ITFluids.HEATED_SALT.getStill(), 80), 640, 10).build(out, toResourceLocation("heat_exchanger/distwater_moltensalt"));
    }

    private void recipesMixer(@Nonnull RecipeOutput out) {
        new MixerRecipeJsonBuilder(ITFluids.SALT_SLURRY.getStill(), 1000)
                .addFluidTag(FluidTags.WATER, 1000)
                .addInput(new IngredientWithSize(ITTags.saltForge, 4))
                .setEnergy(3200)
                .build(out, toResourceLocation("mixer/salt_slurry"));
        new MixerRecipeJsonBuilder(ITFluids.GRAVEL_SLURRY.getStill(), 1000)
                .addFluidTag(FluidTags.WATER, 1000)
                .addInput(new IngredientWithSize(Tags.Items.GRAVELS, 4))
                .setEnergy(3200)
                .build(out, toResourceLocation("mixer/gravel_slurry"));
    }

    private void recipesTurbine(@Nonnull RecipeOutput out) {
        ...
        gasolineBuilder.build(out.withConditions(modLoaded("immersivepetroleum")), toResourceLocation("gas_turbine/gasoline"));
        ...
        dieselBuilder.build(out.withConditions(modLoaded("immersivepetroleum")), toResourceLocation("gas_turbine/diesel"));
        ...
        keroseneBuilder.build(out.withConditions(modLoaded("immersivepetroleum")), toResourceLocation("gas_turbine/kerosene"));
    }

    private void recipesSolarMelter(@Nonnull RecipeOutput out) {
    ...
    private void recipesSolarTower(@Nonnull RecipeOutput out) {
    ...
    private static class MixerRecipeJsonBuilder extends IEFinishedRecipe<MixerRecipeJsonBuilder> {
        private MixerRecipeJsonBuilder(Fluid output, int amount) {
            super(MixerRecipe.SERIALIZER.get());
            addFluid("result", new FluidStack(output, amount));
        }

        public MixerRecipeJsonBuilder addFluidTag(TagKey<Fluid> fluidTag, int amount) {
            return addFluidTag("fluid", fluidTag, amount);
        }

        @Override
        public MixerRecipeJsonBuilder addInput(IngredientWithSize input) {
            return addWriter(json -> {
                JsonArray inputs = json.has("inputs") ? json.getAsJsonArray("inputs") : new JsonArray();
