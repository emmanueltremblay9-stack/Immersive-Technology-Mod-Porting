...
public class CrateCreativeBlock extends ITEntityBlock<CrateCreativeBlockEntity> {
    ...
    @Override public @NotNull ItemInteractionResult useItemOn(@NotNull ItemStack stack, @NotNull BlockState state, @NotNull Level level, @NotNull BlockPos pos, Player player, @NotNull InteractionHand hand, @NotNull BlockHitResult hit) {
        if (level.isClientSide) return ItemInteractionResult.SUCCESS; CrateCreativeBlockEntity tile = (CrateCreativeBlockEntity) level.getBlockEntity(pos); if (tile != null) ((ServerPlayer) player).openMenu(tile, b -> b.writeBlockPos(pos)); return ItemInteractionResult.CONSUME;
    }
}
