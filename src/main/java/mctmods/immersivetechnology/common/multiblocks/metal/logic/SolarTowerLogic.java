...
public class SolarTowerLogic implements IMultiblockLogic<SolarTowerLogic.State>, IServerTickableComponent<SolarTowerLogic.State>, IClientTickableComponent<SolarTowerLogic.State>, ITPressurizedFluidOutput<SolarTowerLogic.State> {
    ...
    private boolean recipeLogic(State state, Level level, boolean enabled) {
        ...
        if (state.activeRecipe == null || !state.activeRecipe.input.ingredient().test(fs)) { state.activeRecipe = SolarTowerRecipe.findRecipe(level, fs); state.processProgress = 0; state.totalProcessTime = 0; if (state.activeRecipe == null) { return false; } }
    ...
    public static class State implements ITISolarMultiblockState, ITDisplayContext {
        ...
        @Nullable private ResourceLocation getActiveRecipeIdForSave() {
            ...
            for (var holder : SolarTowerRecipe.RECIPES.getRecipes(level)) {
