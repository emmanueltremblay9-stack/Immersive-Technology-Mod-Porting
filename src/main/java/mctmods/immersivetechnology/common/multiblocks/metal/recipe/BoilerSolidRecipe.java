...
public class BoilerSolidRecipe extends MultiblockRecipe {
    public static DeferredHolder<net.minecraft.world.item.crafting.RecipeSerializer<?>, IERecipeSerializer<BoilerSolidRecipe>> SERIALIZER;
    ...
    public BoilerSolidRecipe(IngredientWithSize input, double heatPerTick, double targetHeat) {
        super(TagOutput.EMPTY, ITRecipeTypes.BOILER_SOLID, 0, 0, ITRecipeTypes.NO_MULTIPLIERS);
        this.input = input;
    ...
    public static BoilerSolidRecipe findRecipe(Level level, ItemStack input) {
        for (var holder : RECIPES.getRecipes(level)) { BoilerSolidRecipe recipe = holder.value();
