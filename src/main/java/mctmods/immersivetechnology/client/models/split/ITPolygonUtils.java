...
public class ITPolygonUtils {
    ...
    public static Polygon<ITPolygonUtils.ExtraQuadData> toPolygon(BakedQuad quad) {
        ...
        return new Polygon<>(vertices, new ITPolygonUtils.ExtraQuadData(
                quad.getSprite(),
                new Vector4f((color & 255) / 255f, ((color >> 8) & 255) / 255f, ((color >> 16) & 255) / 255f, (color >> 24) / 255f))
        );
    }

    public static BakedQuad toBakedQuad(Polygon<ITPolygonUtils.ExtraQuadData> poly, ModelState transform) {
        return toBakedQuad(poly.getPoints(), poly.getTexture(), transform.getRotation().blockCenterToCorner(), true);
    }

    public static BakedQuad toBakedQuad(List<Vertex> points, ITPolygonUtils.ExtraQuadData data, Transformation rotation, boolean absoluteUV) {
    ...
    private static float[] toArray(Vec3d vec, int length) {
        ...
        for (int i = 3; i < length; ++i) { ret[i] = 1; }
        return ret;
    }
