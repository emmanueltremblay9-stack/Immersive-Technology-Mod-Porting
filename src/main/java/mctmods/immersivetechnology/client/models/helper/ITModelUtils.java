...
public class ITModelUtils {
    public static RenderTypeGroup copyTypes(SimpleBakedModel simpleModel) {
        ...
        if (blockTypes == null || blockTypes.isEmpty()) { return RenderTypeGroup.EMPTY; }
    ...
    public static Transformation fromItemTransform(ItemTransform transform, boolean leftHand) {
        ...
        Quaternionf leftRotation = new Quaternionf().rotateXYZ(Mth.DEG_TO_RAD * leftRX, Mth.DEG_TO_RAD * leftRY, Mth.DEG_TO_RAD * leftRZ);
        ...
        Quaternionf rightRotation = new Quaternionf().rotateXYZ(Mth.DEG_TO_RAD * rightRX, Mth.DEG_TO_RAD * rightRY, Mth.DEG_TO_RAD * rightRZ);
    ...
    @Nonnull public static Set<BakedQuad> createBakedBox(Vec3 from, Vec3 to, Matrix4 matrixIn, Direction facing, Function<Vec3[], Vec3[]> vertexTransformer, Function<Direction, TextureAtlasSprite> textureGetter, float[] colour) {
        ...
        Vec3[] vertices = {new Vec3(from.x, from.y, from.z), new Vec3(from.x, from.y, to.z), new Vec3(to.x, from.y, to.z), new Vec3(to.x, from.y, from.z)};
        TextureAtlasSprite sprite = textureGetter.apply(Direction.DOWN);
        if (sprite != null) { quads.add(createBakedQuad(ClientUtils.applyMatrixToVertices(matrix, vertexTransformer.apply(vertices)), Utils.rotateFacingTowardsDir(Direction.DOWN, facing), sprite, new double[]{from.x * 16, 16 - from.z * 16, to.x * 16, 16 - to.z * 16}, colour, true)); }
        for (int i = 0; i < vertices.length; i++) {
            Vec3 v = vertices[i];
            vertices[i] = new Vec3(v.x, to.y, v.z);
        }
        sprite = textureGetter.apply(Direction.UP);
        if (sprite != null) { quads.add(createBakedQuad(ClientUtils.applyMatrixToVertices(matrix, vertexTransformer.apply(vertices)), Utils.rotateFacingTowardsDir(Direction.UP, facing), sprite, new double[]{from.x * 16, from.z * 16, to.x * 16, to.z * 16}, colour, false)); }
        vertices = new Vec3[]{new Vec3(to.x, to.y, from.z), new Vec3(to.x, from.y, from.z), new Vec3(from.x, from.y, from.z), new Vec3(from.x, to.y, from.z)};
        sprite = textureGetter.apply(Direction.NORTH);
        if (sprite != null) { quads.add(createBakedQuad(ClientUtils.applyMatrixToVertices(matrix, vertexTransformer.apply(vertices)), Utils.rotateFacingTowardsDir(Direction.NORTH, facing), sprite, new double[]{from.x * 16, 16 - to.y * 16, to.x * 16, 16 - from.y * 16}, colour, false)); }
        for (int i = 0; i < vertices.length; i++) {
            Vec3 v = vertices[i];
            vertices[i] = new Vec3(v.x, v.y, to.z);
        }
        sprite = textureGetter.apply(Direction.SOUTH);
        if (sprite != null) { quads.add(createBakedQuad(ClientUtils.applyMatrixToVertices(matrix, vertexTransformer.apply(vertices)), Utils.rotateFacingTowardsDir(Direction.SOUTH, facing), sprite, new double[]{to.x * 16, 16 - to.y * 16, from.x * 16, 16 - from.y * 16}, colour, true)); }
        vertices = new Vec3[]{new Vec3(from.x, to.y, to.z), new Vec3(from.x, from.y, to.z), new Vec3(from.x, from.y, from.z), new Vec3(from.x, to.y, from.z)};
        sprite = textureGetter.apply(Direction.WEST);
        if (sprite != null) { quads.add(createBakedQuad(ClientUtils.applyMatrixToVertices(matrix, vertexTransformer.apply(vertices)), Utils.rotateFacingTowardsDir(Direction.WEST, facing), sprite, new double[]{to.z * 16, 16 - to.y * 16, from.z * 16, 16 - from.y * 16}, colour, true)); }
        for (int i = 0; i < vertices.length; i++) {
            Vec3 v = vertices[i];
            vertices[i] = new Vec3(to.x, v.y, v.z);
        }
        sprite = textureGetter.apply(Direction.EAST);
        if (sprite != null) { quads.add(createBakedQuad(ClientUtils.applyMatrixToVertices(matrix, vertexTransformer.apply(vertices)), Utils.rotateFacingTowardsDir(Direction.EAST, facing), sprite, new double[]{16 - to.z * 16, 16 - to.y * 16, 16 - from.z * 16, 16 - from.y * 16}, colour, false)); }
        return quads;
    }
    ...
    public static ResourceLocation getSideTexture(@Nonnull BakedModel model, Direction side, @Nullable BlockState state) {
        ...
        if (quads.isEmpty()) { quads = model.getQuads(state, null, ApiUtils.RANDOM_SOURCE, ModelData.EMPTY, null); }
